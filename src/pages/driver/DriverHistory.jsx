  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      <div className="bg-white px-6 py-4 border-b border-gray-100 sticky top-0 z-10">
        <h1 className="text-xl font-semibold text-gray-900 mb-4">Riwayat Perjalanan</h1>
      ...
      <div className="px-4 py-4 space-y-3">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 font-medium">Belum ada riwayat perjalanan.</p>
          </div>
        ) : (
          filteredHistory.map(item => (
            <div 
              key={item.id} 
              onClick={() => navigate(`/driver/history/${item.id}`)}
            ...
            >
              ...
              <div className="flex items-center gap-4 text-sm text-gray-600 pt-3 border-t border-gray-50 mt-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <span>{item.rider_count} Penumpang</span>
                </div>
                {item.rating_received && (
                  <div className="flex items-center gap-1.5">
                    <Star className="w-4 h-4 text-yellow-400 fill-yellow-400" />
                    <span className="font-medium text-gray-900">{item.rating_received}</span>
                  </div>
                )}
              </div>
            </div>
      ...
      <BottomNav role="driver" active="riwayat" />
    </div>
  );
}
