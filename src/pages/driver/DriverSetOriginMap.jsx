...
export default function DriverSetOriginMap() {
  const navigate = useNavigate();
  const [position, setPosition] = useState({ lat: -7.9518, lng: 112.6144 });

  const handleConfirm = () => {
    sessionStorage.setItem('driver_origin', JSON.stringify({
      lat: position.lat,
      lng: position.lng,
      label: `Lokasi Asal (${position.lat.toFixed(4)}, ${position.lng.toFixed(4)})`
    }));
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col relative">
      ...
      <div className="absolute bottom-0 left-0 w-full bg-white rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.1)] p-6 z-50 pb-safe">
        ...
        <h3 className="text-gray-500 text-sm font-medium mb-1">Pilih Titik Asal</h3>
